 ...
 */
export function REUSELicensesPlugin(opts: REUSELicensesPluginOptions = {}): Plugin {
	const options = {
		overwriteLicenses: {},
		...opts,
	}
	 ...
	 */
	function neededFields(data: PackageJSON, dir: string) {
	 ...
	 */
	async function findPackage(dir: string): Promise<ReturnType<typeof neededFields> | null> {
		// check cache first
		...
		try {
			...
			const pkg = await findPackage(dirname(dir))
			if (pkg) {
				packageCache.set(dir, pkg as never)
			}
			return pkg
		}
		...
		packageCache.set(dir, neededFields(packageInfo, dir))
		return packageCache.get(dir)!
	}
		...
		async generateBundle(outputOptions, bundle) {
			...
			for (const chunk of Object.values(bundle)) {
				...
				const allPackages = (await Promise.all([...modules.values()]
					.map(sanitizeName)
					.map(findPackage)))
					.filter(Boolean) as ReturnType<typeof neededFields>[]
				...
				const sortedPackages = [...packages].sort((a, b) => a.name.localeCompare(b.name) || (a.version?.localeCompare(b.version ?? '', undefined, { numeric: true }) ?? 0))
